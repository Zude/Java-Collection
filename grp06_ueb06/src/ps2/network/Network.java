...
public class Network {
    ...
    public Network rmvLocation(Location loc) {
        ...
        if (!this.locationExist(loc)) {
            ...
            for (Connection connection : this.connections) {
                if ((connection.from != loc) && (connection.to != loc)) {
                    newConnections[index2] = connection;
                    index2++;
                }
            }
    ...
    public Network addConnection(Connection c, Location from, Location to) {
        ...
        if (this.connections.length == 0) {
            ...
            Connection[] myArray = Arrays.copyOf(this.connections, this.connections.length);
            myArray[myArray.length] = c;
            this.connections = myArray;
    ...
    public Network addConnection(Connection c, String from_key, String to_key) {
        ...
        Connection[] myArray = new Connection[this.connections.length + 1];
        System.arraycopy(this.connections, 0, myArray, 0, this.connections.length);
        myArray[this.connections.length] = c;
        this.connections = myArray;
