...
public class Location {
    ...
    private Connection[] filterArray(Location loc, Connection[] myCon) {

        if (myCon.length != 0) {
            for (Connection myCon1 : myCon) {
                if (myCon1.from.equals(loc) || myCon1.to.equals(loc)) {
                    myCon = this.rmvArrayConnection(myCon, myCon1);
                }
            }
        }
        return myCon;
    ...
    public void rmvConnectionsWith(Location loc) {
        // TODO DONE: Schreibt exakt eine Methode, die alle Vorkommen einer Location
        //       in einem Connection-Array löscht
        // this.outbound = filterArray(loc, this.outbound);
        // this.inbound = filterArray(loc, this.inbound);
